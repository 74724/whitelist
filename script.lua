...
repeat
    success, err = pcall(function()
        ...
        for _, form in ipairs(PaidFormsList["Good"]) do
            task.wait()
            print(string.format(
                "Name: %s, ReqValue: %d, EndRangeValue: %d, Alignment: %s, RebirthReq: %d, FormOwned: %s",
                    ...
                    for j = i - 1, 1, -1 do -- search for the previous quest
                        ...
                        if planet1 == prevQuest.planet then
                            ...
                            return SelectedQuests, SelectedMobs,
                                sameplanet                                  -- exit the loop once the previous quest is found
                        else
                            sameplanet = false
...
spawn(function()
    ...
    while getgenv().AutoBlock do
        if not getgenv().AutoBlock then                                                  -- condition to check if blokcing is on
            game:GetService("ReplicatedStorage").Package.Events.block:InvokeServer(true) -- blocks on exit
            task.wait()                                                                  --waits every frame time
            break                                                                        --exits out of loop --once exited not re-enterable
        end

        if not game:GetService("Players").LocalPlayer.Status.Blocking.Value then         -- blocking isnt happening
            game:GetService("ReplicatedStorage").Package.Events.block:InvokeServer(true) --blocks
        end
        ...
        if getgenv().stackneeded == true then 
            ...
            if getgenv().stackneeded == true then
                ...
                repeat
                    ...
                    if success then
                        warn("(1)Stack loop Function executed successfully with a value of " ..
                        tostring(getgenv().stackneeded))
